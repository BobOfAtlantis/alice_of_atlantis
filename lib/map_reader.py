...
def load_map_data(obs, args):
    ...
    if os.path.isfile(file_name) is True:
        ...
        if "screen height chart" in file_data:
            bot.screen_height_chart = np.array(file_data["screen height chart"])

        if "minimap offset chart" in file_data:
            bot.minimap_offset_chart = np.array(file_data["minimap offset chart"])

        if "command home base" in file_data:
            bot.command_home_base = file_data["command home base"]
        if "minimap home base" in file_data:
            bot.minimap_home_base = file_data["minimap home base"]
        if "screen height chart offset" in file_data:
            bot.screen_height_chart_offset = file_data["screen height chart offset"]
        if "minimap select area" in file_data:
            bot.minimap_select_area = file_data["minimap select area"]
...
def issue_move_action_on_screen(obs, args):
    ...
    if screen_location is None:
        print("What? that's not on the screen")
        return None
...
def match_screens(screen1, screen2, shift):
    ...
    if np.array_equal(screen1, screen2):
        print("successfully recursively matched screens the shift is: " + str(shift))
        return shift
    # haven't thought too much about the constant 20 here, lot's of work to search a square
    elif abs(x) > 20 or abs(y) > 20:
        return None
    else:
...
# 4) Determine how many screen pixels are in a minimap pixel
...
def calibrate_map_data(obs, args):
    ...
    if single_select[0][0] == static.unit_ids["command center"]:
        ...
        ys, xs = np.nonzero(unit_type == static.unit_ids['command center'])
        ...
        # bot.absolute_home_base = [bot.minimap_pixel_size[0] * bot.command_home_base[0],
        #   bot.minimap_pixel_size[1] * bot.command_home_base[1]]
    ...
    for y in range(len(area)):
        for x in range(len(area[0])):
            if area[y][x] == 0:
                ...
                chart_list.append([dist_x+dist_y, x + min_x, y + min_y])  # adjust list from relative to absolute
...
def get_absolute_location(obs, args):
    ...
    if "point" in args:
        point = args["point"]
    x, y = point
    # The minimap pixels don't line up correctly with the screen pixels,
    # this method gets you close to the correct absolute position, it's slightly off.
...
def start_height_chart(obs, args):
    ...
    if step == 1:
        ...
        # we have to re-save the minimap_home_base here, because after moving the screen,
        # it is possible that the edge of the map got in the way and the center we use from here out is new
...
def update_height_chart(obs, args):
    ...
    if step == 0:
        ...
        if bot.minimap_offset_chart[0, y, x-1] == 1:
            ...
            # the top left pixel of the chunk is minimap_offset_
            #  chart[1,point[0]-1,point[0]] - screen_height_chart_offset
            ...
            for i in range(30):
                ...
                # check to see if the height data are equal.
                # print("trying i of: " + str(i) + " old shape: " + str(np.shape(old_height_chunk))
                #  + " new shape: " + str(np.shape(new_height_data)))
                ...
                if np.array_equal(old_height_chunk, new_height_data):
                    ...
                    new_height_copy = new_height_copy[
                                      0:np.shape(new_height_copy)[0],
                                      np.shape(new_height_copy)[1] - (i+1):np.shape(new_height_copy)[1]
                                    ]
                    ...
                    print(f"right edge of chart: {chart_offset_x} + bot.screen_dimensions[0] {bot.screen_dimensions[0]}"
            ...
            # load the old_height_data from the screen_height_chart for the correct spot
            # the we need an 84x84 height chunk from screen_height_chart
            # the top left pixel of the chunk is
            #   minimap_offset_chart[1,point[0]-1,point[0]] - screen_height_chart_offset
            chart_offset_x = bot.minimap_offset_chart[1, y, x+1]
            chart_offset_y = bot.minimap_offset_chart[2, y, x+1]
            global_offset_x = bot.screen_height_chart_offset[0]
            global_offset_y = bot.screen_height_chart_offset[1]
            ...
            for i in range(30):
                ...
                # check to see if the height data are equal.
                # print("trying i of: " + str(i) + " old shape: " +
                #   str(np.shape(old_height_chunk)) + " new shape: " + str(np.shape(new_height_data)))
                ...
                if np.array_equal(old_height_chunk, new_height_data):
                    ...
                    # new_columns is how much space we need left of the chunk
                    # self.screen_height_chart_offset[0] describes the left most edge of the existing height chart
                    # print(f"left edge of chart: {chart_offset_x}
                    #   - new columns {new_columns} vs global {global_offset_x}")
                    ...
                    if new_data_left_edge < 0:
                        ...
                        # if we are scanning from the right,
                        #  update the screen_height_chart_offset with the new offset info
                        # now update the minimap_offset_chart
            ...
            # load the old_height_data from the screen_height_chart for the correct spot
            # the we need an 84x84 height chunk from screen_height_chart
            # the top left pixel of the chunk is minimap_offset_chart[1,point[1],point[0]-1]
            # - screen_height_chart_offset
            ...
            for i in range(30):
                ...
                if np.array_equal(old_height_chunk, new_height_data):
                    ...
                    # get the new data to be added
                    new_height_copy = new_height_copy[
                                      np.shape(new_height_copy)[0] - (i+1):np.shape(new_height_copy)[0],
                    ...
                    print(f"bottom edge of chart: {chart_offset_y} + screen_dimensions[1] {bot.screen_dimensions[1]}"
                          f" + new_rows {new_rows}")
            ...
            # the we need an 84x84 height chunk from screen_height_chart
            # the top left pixel of the chunk is minimap_offset_
            #   chart[1,point[1],point[0]-1] - screen_height_chart_offset
            chart_offset_x = bot.minimap_offset_chart[1, y+1, x]
            chart_offset_y = bot.minimap_offset_chart[2, y+1, x]
            global_offset_x = bot.screen_height_chart_offset[0]
            global_offset_y = bot.screen_height_chart_offset[1]
            ...
            for i in range(30):
                ...
                if np.array_equal(old_height_chunk, new_height_data):
                    ...
                    if new_data_top_edge < 0:
                        ...
                        # if we are scanning from the right, update the
                        #   screen_height_chart_offset with the new offset info
                        # now update the minimap_offset_chart
                        print(f"setting new global offset: {global_offset_y} - {new_rows}")
